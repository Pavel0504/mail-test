...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!emailTextContent && !emailHtmlContent || !emailSubject) {
      const groupMembersRes = await fetch(`${supabaseUrl}/rest/v1/contact_group_members?contact_id=eq.${contact.id}&select=group_id`, {
        headers: {
          "apikey": supabaseServiceKey,
          "Authorization": `Bearer ${supabaseServiceKey}`
        }
      ...
      if (groupMembers && groupMembers.length > 0) {
        ...
        const groupRes = await fetch(`${supabaseUrl}/rest/v1/contact_groups?id=eq.${groupId}&select=default_subject,default_text_content,default_html_content`, {
          headers: {
            "apikey": supabaseServiceKey,
            "Authorization": `Bearer ${supabaseServiceKey}`
          }
    ...
    const { data: currentMailing } = await fetch(`${supabaseUrl}/rest/v1/mailings?id=eq.${mailingId}&select=sent_count,success_count`, {
      headers: {
        "apikey": supabaseServiceKey,
        "Authorization": `Bearer ${supabaseServiceKey}`
      }
    ...
    const { data: pendingRecipients } = await fetch(`${supabaseUrl}/rest/v1/mailing_recipients?mailing_id=eq.${mailingId}&status=eq.pending&select=id`, {
      headers: {
        "apikey": supabaseServiceKey,
        "Authorization": `Bearer ${supabaseServiceKey}`
      }
    ...
    if (recipient_id) {
      ...
      try {
        const { data: recipientData } = await fetch(`${supabaseUrl}/rest/v1/mailing_recipients?id=eq.${recipient_id}&select=mailing_id,sender_email_id`, {
          headers: {
            "apikey": supabaseServiceKey,
            "Authorization": `Bearer ${supabaseServiceKey}`
          }
        ...
        if (recipientData) {
          ...
          // Обновляем счетчики рассылки
          const { data: currentMailing } = await fetch(`${supabaseUrl}/rest/v1/mailings?id=eq.${mailingId}&select=sent_count,failed_count`, {
            headers: {
              "apikey": supabaseServiceKey,
              "Authorization": `Bearer ${supabaseServiceKey}`
            }
          ...
          if (!pendingRecipients || pendingRecipients.length === 0) {
            const { data: mailingStats } = await fetch(`${supabaseUrl}/rest/v1/mailings?id=eq.${mailingId}&select=success_count,failed_count`, {
              headers: {
                "apikey": supabaseServiceKey,
                "Authorization": `Bearer ${supabaseServiceKey}`
              }
          ...
          // Обновляем счетчики почты
          if (senderEmailId) {
            const { data: emailData } = await fetch(`${supabaseUrl}/rest/v1/emails?id=eq.${senderEmailId}&select=sent_count,failed_count`, {
              headers: {
                "apikey": supabaseServiceKey,
                "Authorization": `Bearer ${supabaseServiceKey}`
              }
